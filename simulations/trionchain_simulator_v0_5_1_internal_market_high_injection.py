...
# ============================================================
#  TRIONCHAIN SIMULATOR v0.5.1
#  FEM Mesh + Internal Energy Market (HIGH INJECTION)
# ============================================================
...
class TrionCell:
    def __init__(self, eid, node_indices, coords):
        ...
        # Optional "drivers" (used to shape demand)
...
# ---------------------------
# Injection schedule (MW into chain)
# ---------------------------
def injection_schedule(block, base_total_mw=900.0):
    # deterministic smooth variability
...
# ---------------------------
# Neighbor redistribution (simple "power diffusion")
# ---------------------------
def neighbor_flow_step(cells, alpha=0.20):
    ...
    # start each block from injections only
    for c in cells.values():
        c.available_mw = c.injection_mw

    # one pass, from surplus to deficit neighbors
    for cid, c in cells.items():
...
# ---------------------------
# Internal market (sell surplus to neighbor deficits)
# ---------------------------
def internal_market_clearing(cells, base_price=50.0, k_scarcity=30.0):
    ...
    for sid in sellers:
        ...
        if not neighbor_buyers:
            seller.surplus_inventory_mwh += seller.surplus_mw  # 1 block ~ 1h
...
def run_simulation(
    ...
    steps=60,
    injection_nodes_k=3,
    base_total_mw=900.0,   # <<< CHANGED: higher injection
    alpha_flow=0.20,
    base_price=50.0,
    k_scarcity=30.0,
    seed_rwa=42,
...
def plot_results(series):
    ...
    plt.title("Boundary Mismatch (Flows) – Internal Market (High Injection)")
...
def save_json(series, filename="trionchain_internal_market_high_injection_results.json"):
...
if __name__ == "__main__":
    print("\nTrionChain v0.5.1 – FEM Internal Energy Market (High Injection)\n")
